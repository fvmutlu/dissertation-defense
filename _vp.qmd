...
**Approach:** Extending VIP @yeh2014vip, minimize upper bound of [drift-plus-penalty]{.body-highlight}[^dpp] expression:
```{=tex}
\begin{equation*}
    \drift + \omega \pen
\end{equation*}
...
**Algorithm:** Distributed and adaptive; solve caching and forwarding subproblems at each node, beginning of each time slot, based on current backlogs.
...
::: {style="font-size: 80%;"}
$L_{n_j}$: Capacity of tier $\jin$ in objects.
:::
...
::: {style="font-size: 85%;"}
We can rewrite [caching]{.body-highlight} subproblem as a *multiple knapsack problem*.
:::
...
## Lemma: Caching Solution {visibility="uncounted"}
...
- Global optimum can be found in polynomial time via the Hungarian method or its efficient variants.
...
Optimal solution to [forwarding]{.body-highlight} problem is obtained by choosing $\mu^k_{ab}(t)$ as follows.
...
This is the well-known ***backpressure*** solution.
...
::: {style="font-size: 120%;"}
Algorithm achieves following [backlog-penalty trade-off]{.body-highlight} for arrival rate vector $\boldsymbol{\lambda} \in \textup{int}(\Lambda)$:
:::
...
$\bar{P}(\boldsymbol{\lambda})$: Minimum expected time-average sum penalty achievable by feasible stationary policy.
:::
