...
-   Data-intensive science experiments (high-energy physics, genomics etc.) process huge amounts of data
    -   In one year of LHC running, over an exabyte ($10^{18}$ bytes) of data is accessed
    -   In November 2018, 16 petabytes ($1.6 \times 10^{16}$ bytes) of data were written on tape at CERN
...
::: {.incremental}
-  DRAM is the primary cache device since it is fast (32-64 GB/s for DDR5)
-  However, DRAM offers small capacities at a high and exponentially increasing cost ($3 to $12 / GB)
-  Next best option is NVMe SSD, offering large capacities at significantly lower costs ($0.1 / GB)
-  NVMe SSDs are much slower than DRAM (11.7 GB/s for fastest PCIe 5.0)
:::
...
-  Benefits of caching more pronounced when caching and forwarding decisions are tightly coupled
   -  "*Coupling caching and forwarding: benefits, analysis, and implementation*", G. Rossini, D. Rossi, ICN 2014
...
- (Fragment in) So in this way, we're not trying to make a precisely optimal decision each time slot, but rather we're trying to minimize the upper bound on this drift-plus-penalty expression you see on screen, which is based on an extension to Lyapunov optimization devised by Neely
:::
...
Perform [forwarding]{.body-highlight} as in (Yeh, 2014), by choosing $\mu^k_{ab}(t)$ for each $\kin$ and $\abin$:
...
::: {.notes}
- Now recall the assumption we made in our model that every data object has the same size, because it is critical here.
...
## Forwarding Solution
*Backpressure* solution to forwarding optimization problem as in (Yeh, 2014).
```{=tex}
\begin{equation}
   \mu^k_{ab}(t) =
   \left\{ \begin{array}{ll}
...
[Oscillatory]{.body-highlight}[^oscillatory] nature of VIP counts impractical for data plane caching. Adapt [cache score]{.body-highlight} metric from (Yeh, 2014), defined over sliding window of $T$ slots:
```{=tex}
\begin{equation}
...
$\mc{v}^k_{ab}$: Number of VIPs for $k$ transmitted over $(a,b)$ during $t$

[^oscillatory]: When the VIP count for an object becomes large, we make decisions in the virtual plane that will quickly drive the count down.
...
- So we adapt this cache score metric
:::
...
$\mc{K}_{n_j}(t)$: Set of objects cached in $j$ at $n$ during $t$, in the data plane
...
When $k \not \in \mc{K}_{n_j}(t)$ arrives at $n$ during $t$, [data plane caching policy]{.body-highlight} at $n$ behaves as follows:

-  Find cache tier offering highest cache benefit, i.e. $j^* = \argmax_{\{ \jin \}} CB^k_{n_j}(t)$
...
- N-DISE (led by Yuanhao Wu and Prof. Yeh):
    - Helped build and test N-DISE WAN deployment
    - Helped with demonstrations during SC19, SC21 and SC22
    - Built local testbed for prototyping and preliminary experiments
- Energy minimization in HetNets (led by Jinkun Zhang, small contribution)
