...
- **Context**: Information-centric networking (ICN), content delivery networks, in-network caching
...
::: {.notes}
- ICN: Move Internet from communication network to data distribution network by making data the primary entity
- Work is applicable in traditional CDN contexts, could be used as an overlay
- In-network caching: Enable every router to maintain its own cache to move away from centralized caching
:::
...
::: {.notes}
To expand on our motivation, let's begin with the use case that inspired my work
:::
...
::: {.notes}
- N-DISE paper appeared in ACM ICN 2022
- NDN-DPDK forwarder is a state-of-the-art high-throughput NDN forwarder and a core enabler of the N-DISE system, and in the paper that introduced that forwarder the need for specialized policies to operate SSD-based larger caches was outlined
:::
...
-  Catalog of 1000 objects, ranked in popularity according to [Zipf's law]{.body-highlight}
...
-  Caches are not free to use: admissions, replacements, even idle operation has a cost
-  With additional cache devices, these are more pronounced:
   -  SSDs wear out over time as they're written
   -  Slow transfers mean cache state transitions take longer
...
::: {.notes}
- The most significant operational cost is power
- Difficult to represent the costs of idle operation, but we model admission and replacement costs as we'll see
:::
...
::: {.notes}
- Recall the earlier paper I mentioned about coupling caching and forwarding, coincidentally the two papers appeared in the same conference.
:::
