...
- And I needed to build this because existing simulators didn't cover what we needed, and were missing some features I wanted and modifying them would've taken even more time
...
::: {.notes}
- And there are some additional assumptions that pertain to the experiment setting so let's go over those
- (Talk about bullet points)
- Cost-aware LFU adaptation uses a cache benefit metric similar to that we devised for our approach
...
- However, anywhere between 15% to 20% of cache hits are occurring on the first tier with our approach, showing that it is able to balance the amount of cache hits better
...
- We'll be looking at how each policy balances the delay vs. penalty trade-off
...
- The reason for this again connects to the challenges in managing these larger, slower caches I talked about in the very beginning. This in itself is a long discussion, but the gist is that I wanted to represent the best of the competing policy here.
...
\end{equation}
```
Useful in view of [interest aggregation]{.body-highlight}[^+] and [hardware limitations]{.body-highlight}, but very restrictive:

-  Hardware limitations may not be there for long (peer-to-peer DMA); without interest aggregation, duplicates across tiers could extend bandwidth
...
However, technical analysis is not sufficient to evaluate the impact of removing exclusivity; extensive testing required.
