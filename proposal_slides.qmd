...
::: {.notes}
- So here's a brief overview about my primary research that I will present today.
- First off, for context, my work is based on the Information-centric Networking paradigm, or ICN for short.
- Internet today is built on communication network principles, whereas its primary use is data distribution, so ICN aims to steer it in that direction by making data the primary entity of the network, rather than endpoints.
...
- However, my work is also applicable within today's Internet, especially for systems like CDNs, as it could be used in an overlay.
- The main motivation for my work is that the volume of data we're trying to distribute with these networks continuously grows, but the capacities of our caches are somewhat stagnant.
- There are many obstacles in the way of scaling cache capacities. Specifically we'll look at these three considerations: (1) are slower storage elements viable as caches? (2) what are the costs of operating large caches? (3) how can we get the most out of caches in the network?
:::
...
- For instance, for the LHC, in one year, over an exabyte of data is accessed. In only one month even back in 2018, 16 PB of data were written on permanent storage.
- This data then has to constantly be distributed across the world for research, which is a significant networking challenge.
:::
...
::: {.notes}
- To address this, our lab led a project effort to build a state-of-the-art, Named Data Networking based data distribution system, which you can read about in the paper we published in ICN 2022.
- The point here is, even in this state-of-the-art project, we were only able to allocate a small amount of cache space at each router. As you can see it is a drop in the bucket compared to the volumes we're working with.
- To drive the point home further I also want to briefly mention that, in a separate paper that describes a core technology that enabled the N-DISE system (the NDN-DPDK forwarder), the problem of scaling cache capacities and the need for pecialized policies for operating larger caches was specifically outlined.
- So that gap is what motivated my work.
:::
...
::: {.notes}
- I now want to give you a better idea about why scaling capacities is a challenge.
:::
...
-  Beginning of each time slot, each node decides where and how to forward its VIPs
...
- So in this way, we're not trying to make a precisely optimal decision each time slot, but rather trying to minimize the upper bound on this drift-plus-penalty expression you see on screen, which is based on an extension to Lyapunov optimization devised by Neely
:::
...
::: {.notes}
- The analysis of our approach comes in two steps. If you recall my statement about Lyapunov drift analysis, we're following that structure here to first define the stability region of the network of VIP counts (which can be seen as queues) in the virtual plane.
...
::: {.notes}
- As for the solution to the forwarding step, a neat feature of our extension here is that we don't actually alter the results for forwarding from the original VIP paper.
...
::: {.notes}
- Cache score is based on a sliding window average of received VIPs
:::
...
::: {.notes}
- Cache benefit balances cache score against weighted penalty
:::
...
-  Nodes generate requests for 100 seconds
...
::: {.notes}
Cost-aware LFU adaptation uses a similar cache benefit metric
:::
...
::: {.notes}
Important parameters: number of tiers, capacity of first tier
:::
...
-  Combined with multi-tiered caches, raises the [integrity]{.body-highlight} question, i.e. should we require that nodes cache all chunks of an object in the same tier?
