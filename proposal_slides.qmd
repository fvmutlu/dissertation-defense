...
::: {.incremental}
-  **Motivation**: Data volume is growing exponentially, but capacities of in-network caches are stagnant
-  **Challenge**: Enabling larger caches in a cost-effective way is not trivial
-  **Considerations**:
...
## Coupling Caching with Forwarding* {.smaller}

::: {.incremental}
-  Benefits of ubiquitous caching offered by ICN architectures are more pronounced when caching and forwarding decisions are tightly coupled
   -  "*Coupling caching and forwarding: benefits, analysis, and implementation*", G. Rossini, D. Rossi, ACM ICN 2014
-  The need for this coupling is even more prevalent with larger, slower caches
   -  Based on current demand in different regions of the network, a cache miss at a node could be directed towards a closer node which caches the object in a slower tier or a further node which does so in a faster tier
:::
...
## Model Notation {.smaller .scrollable}
...
Rather than aiming to make the optimal choice for all $s^k_{n_j}(t)$ and $\mu^k_{ab}(t)$ at each $t$, our objective is to minimize the following *drift-plus-penalty* expression:
```{=tex}
\begin{equation}
    \drift + \omega \pen
\end{equation}
...
\end{equation}
```
**Interpretation**: We want to minimize a combined bound of time-average queue backlog growth and accumulated penalty.
...
**Stability Region**: We say that the queue $V^k_n(t)$ is stable if:
```{=tex}
\begin{equation}
\begin{split}
    \limsup\limits_{t \rightarrow \infty} \frac{1}{t} \sum^t_{\tau = 1} 1_{[V^k_n(\tau) > \xi]} d\tau \rightarrow 0 \; \text{as} \; \xi \rightarrow \infty
\end{split}
\end{equation}
```
...
## Chunk-level Decisions {.smaller}

Note that our model describes the data plane in object-level terms as well. When applying our approach at the chunk-level in the data plane, we observe the following principles:

-   If a data object is admitted to a cache tier, all its chunks must be admitted to the same tier.
-   If a data object is evicted from a cache tier, all its chunks must be evicted from that tier.
-   The forwarding decision described in (26) is made when a request for the first chunk of object $k$ arrives at node $n$. Requests for subsequent chunks of $k$ are forwarded to the same node, until the ending chunk is forwarded, after which time if a first chunk request arrives again, the decision will be made again.
