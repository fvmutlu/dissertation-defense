...
[• Requests can enter network at any node; forwarded toward content sources.]{.fragment fragment-index=1}

[• Responses carry data back, following path of request in reverse.]{.fragment fragment-index=2}

[• Nodes along the path can respond from their cache.]{.fragment fragment-index=3}
...
[• Cache operations are ***non-blocking*** and can be queued.]{.fragment fragment-index=2}

[• Each admission or eviction has an associated ***utilization cost*** (or ***penalty***).]{.fragment fragment-index=3}

[• An object evicted from one tier can ***migrate*** to another tier at the same node.]{.fragment fragment-index=4}
:::
...
::: {.incremental}
- Discrete-time queuing system driven by data plane request arrivals; *virtual interest packets* ([*VIPs*]{.body-highlight}) generated along exogenous requests.
- Every node has a [VIP counter]{.body-highlight} for every object $\kin$.
- At the beginning of each slot:
    - Nodes update neighbors with their counts using small messages.
    - Virtual plane algorithm decides cache placement and transmission allocations.
    - VIPs removed (at caches or sources) and forwarded, counters updated.
:::
...
::: {.notes}
- Now, here's where the VIP framework comes in.
- This framework places a virtual control plane on top of the data plane.
- The control plane is a discrete time queuing system which is driven by request arrivals into the data plane.
- Anytime an exogenous request enters the data plane network, a virtual interest packet (or VIP) is generated in the virtual plane at that node.
- Each node maintains a VIP counter for each object in the network, representing the demand for that object at the node.
:::

## VIP Queue Dynamics
[VIP queue dynamics]{.body-highlight} with multi-tiered caching for object $\kin$ at node $\nin$:
