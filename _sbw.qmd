...
- **Reasoning:** Objects in cache may receive different hit rates in the data plane.
...
- **Algorithm:** Should guarantee stability of all VIP queues, minimize total backlog.
    - Minimize upper bound of drift $\drift$.
...
::: {style="font-size: 80%;"}
Given $k_1, k_2, \cdots, k_{J_n}$ such that $V^{k_1}_n(t) \geq V^{k_2}_n(t) \geq ... \geq V^{k_{J_n}}_n(t)$, optimal solution is obtained by ***greedily*** choosing $r\knjt$ and $s\knjt$ for each $\jin$ as follows.
...
## Stability & Throughput Optimality {.smaller}

VIP stability region $\Lambda$ under ***cache bandwidth constraints***:

```{=tex}
\begin{equation*}
\Lambda = \{ \boldsymbol{\lambda} = (\lambda^k_n)_{\kin,\nin} : \lambda^k_n \; \leq \; \sum_{\bin} f^k_{nb} - \sum_{\ain} f^k_{an} + \betasum f^k_{n,i}, \; \forall \kin, \nin \}
\end{equation*}
```

For arrival rate vector $\boldsymbol{\lambda} + \boldsymbol{\epsilon} \in \Lambda$, with $\boldsymbol{\epsilon} > \boldsymbol{0}$, algorithm achieves:
...
$f^k_{n,i}$: Expected removal rate of VIPs of $k$ via caching given placement $\mc{B}_{n,i}$.

::: {layout="[[20,80]]" style="font-size: 80%;"}
```{=tex}
\begin{equation*}
...
::: {.notes}
- Because we control cache read rates more precisely, we can use the additional average flow information generated in the data plane.
- We alter our admission policy to pick the most profitable tier to potentially cache an object based on this information.
- We also compare this with flow averages over links to determine whether it is better to leave the object uncached so future requests continue being forwarded upstream.
:::

## Policy Performance

::: {.incremental}
- This approach yields poor performance in practice.
...
::: {.notes}
- Now, while I'd believed this policy could improve the usage of cache and bandwidth resources and improve delay, in practice it yielded less than desirable results.
- Particularly, the experimental data showed a significant underitilization of the slower cache tier.
- Looking into virtual plane metrics, we concluded the outcome is related to frequent queue underflows, or alternatively, overallocation of service.
- Basically, due to the nature of the max-weight caching backpressure forwarding approach, only a small subset of objects are allocated service in the virtual plane each slot.
- This means that, especially for less popular objects, we are unable to gather flow information quickly.
- Which in turn means the data plane policy may not utilize the secondary tier to a desirable extent. 
:::
...
::: {.notes}
- In order to address this and test whether we can improve our results in practice, we implemented an iterative greedy algorithm that extends the max-weight / backpressure solution.
:::

## Iterative Approach {visibility="uncounted"}
...
- Under ***heavy congestion***, where underflows do not occur, reduces to max-weight / backpressure approach.
...
::: {.notes}
- This algorithm has a couple of notable properties.
- Firstly, when there is heavy congestion in the network such that underflows are not possible, it is equivalent to our initial approach.
- Which means that, at least in such conditions, the throughput optimality result applies under this algorithm.
- Its computational complexity is also unchanged, since the dominating part remains the sorting step.  
:::

<!-- ## Iterative Approach: Properties
...
- **MVIP-RR:** Poor utilization of larger cache tier and resulting performance.
- **MVIP-RR+:** Improved delay performance along with great reduction in replacements.
:::
...
- **MVIP-RR:** Poor utilization of larger cache tier and resulting performance.
- **MVIP-RR+:** Comparable delay performance along with great reduction in replacements.
:::
