...
::: {style="font-size: 75%;"}
- Under [heavy congestion]{.body-highlight} where $V\knt \geq \sum_{\nbin^k} C_{bn} + \max\limits_{\jin} R_{n_j}$, for all $t \geq 1, \nin, \kin$, greedy algorithm is equivalent to *backpressure* solution.
- In general, given the same backlog state $\mathbf{V}(t)$, following holds
```{=tex}
\begin{equation*}
\begin{split}
...
\end{equation*}
```
where $\hat{F}\knjt$ and $\hat{F}^k_{nb}(t)$ denote actual VIP flow under greedy algorithm allocations.
:::
