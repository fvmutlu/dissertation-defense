...
::: {style="font-size: 80%;"}
Given $k_1, k_2, \cdots, k_{J_n}$ such that $V^{k_1}_n(t) \geq V^{k_2}_n(t) \geq ... \geq V^{k_{J_n}}_n(t)$, optimal solution to revised [caching problem]{.body-highlight} is obtained by choosing $r\knjt$ and $s\knjt$ for each $\jin$ as follows.
...
::: {style="font-size: 90%;"}
This is similar to *backpressure* forwarding solution from earlier.
:::
...
$F\knjt$: Actual amount of VIPs of $k$ removed via tier $j$ read rate allocation in slot $t$.
:::
::: {.column width=50% style="font-size: 85%;"}
...
## Policy Performance {.smaller}

::: {.incremental}
- This approach yields poor performance in the data plane.
    - [Significant underutilization]{.body-highlight-red} of slow tier.
- We attribute this outcome to virtual plane ***queue underflows***.
    - Occur when service allocated service exceeds backlog, e.g. $r\knjt > V\knt$.
    - Virtual plane allocations go unused; inhibits actionable flow information for less popular objects.
- Allocation problem under ***no-underflow*** constraints is difficult to solve [^pmw].
    - Backpressure solution no longer *feasible*.
    - Caching and forwarding allocations tied together due to constraint.
    - Non-convex problem; computationally intensive to get approximations.
:::

[^pmw]: "Utility optimal scheduling in processing networks", Huang, L., & Neely, M. J., Performance Evaluation, 2011 @huang2011utility.
...
::: {.column width=50%}
::: {style="font-size: 60%;"}
We can use an iterative greedy algorithm:

- First allocate cache space and read rate to highest backlog objects.
    - Time complexity dominated by sorting, worst case order of $O(K \log K)$.
- Then allocate link bandwidth to largest backlog differential objects.
    - Complexity of $O(DK \log DK)$ where $D$ is a given node's degree.
- ***Feasible***, i.e. does not allocate more service than existing backlog. Terminates when:
...
- Under ***heavy congestion*** where $V\knt \geq \sum_{\nbin^k} C_{bn} + \max\limits_{\jin} R_{n_j}$, for all $t \geq 1, \nin, \kin$, reduces to standard *backpressure*.
- Given backlog state $\mathbf{V}(t)$, following holds in general.
```{=tex}
\begin{equation*}
\begin{split}
...
\end{equation*}
```
- Computational complexity remains the same; sorting dominates.

<hr>

::: {style="font-size: 80%;"}
$F\knjt$, $F^k_{nb}(t)$: Actual VIP flows under standard *backpressure*.
...
- Greedy algorithm maintains much smaller total VIP backlog.
:::
:::
...
:::
::: {.column width=50% style="font-size: 75%"}
- Data plane policy supported by initial approach: MVIP-RR; by greedy algorithm MVIP-RR+.
- Data plane backlog represented by PIT queue sizes.
    - Sampled along with VIPs at each virtual plane time slot.
    - Denote samples as $Q\knt$.
- Greedy algorithm maintains smaller virtual-data plane backlog *"gap"*.
