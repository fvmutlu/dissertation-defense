...
::: {.notes}
- This slide shows a much expanded version of results that were seen in the proposal, where we've tested each possible combination of capacity allocations across the two tiers given the maximum budget, disregarding penalty weights for the time being.
...
- Here, single-tiered implementations of baseline policies are also included and the comparison is such that maximum capacity budget is allocated for these implementations.
- In contrast for two-tiered counterpats, only the second tier is expanded, until it is no longer effective in reducing delay.
:::
