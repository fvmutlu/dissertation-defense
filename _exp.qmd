...
::: columns
::: {.column width="50%"}
![GÉANT GN4-3N (34 nodes) [^geant]](images/geant.svg){width="100%"}
:::
...
![](images/size_comp_abilene_075_15_delay.svg){width="50%" height="50%" fig-align="center"}
...
- In summary, we see that the MVIP strategy outperforms baselines across the board.
:::
...
::: {.notes}
- Of course we also conducted many experiments across other topologies.
- This slide shows a summary of one experiment where we've tested each possible combination of capacity allocations across the two tiers given the maximum budget.
- The values represent the reduction in delay normalized to delay with no caching.
- One takeaway from the figure is that MVIP still outperforms most baselines across the board, with LFU again being a very close competitor.
...
::: {style="font-size: 80%; text-align: center"}
Delay with increasing cache capacities under IA. 
